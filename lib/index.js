...
module.exports = function (ops) {
    ...
    initializeNPM(function() {
      ...
      dependencies = _.map(_.filter(installedModules, function(d) {
        ...
        var moduleConf = _.filter(options.modules, function(m) { return m.name === d.name; });
        moduleConf = moduleConf[0] ? moduleConf[0] : {};
        var loop = function() {
          if(moduleConf.once === true) {
        ...
        var update = function() {
          npm.commands.outdated(d.name, '--silent', function(err, data) {
            ...
            if(data.length === 0) {
              if(moduleConf[0] && moduleConf[0].url) {
                installPackage(d.name, moduleConf[0].url, packagejson, loop);
              } else {
                loop();
              }
